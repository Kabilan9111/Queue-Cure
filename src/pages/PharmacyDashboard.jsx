...
import { Pill, Search, ChevronDown, CheckCircle2, Clock, AlertCircle, Activity, Bell } from 'lucide-react';
...
export default function PharmacyDashboard() {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      ...
      case 'Waiting':
        return <span className="bg-blue-50 text-blue-500 border border-blue-100 px-2.5 py-1 rounded-md text-[10px] uppercase tracking-wider font-bold">Waiting</span>;
      case 'Completed':
      ...
      <header className="h-[80px] bg-white border-b border-[#EEF2FF] flex items-center justify-between px-8 lg:px-10 shrink-0 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-[#6C5CE7] font-bold tracking-wide mt-0.5">Pharmacy Portal</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 cursor-pointer group pl-2 border-l border-slate-100">
            <div className="text-right pr-1">
              <p className="text-[13px] font-bold text-slate-800 leading-tight">Pharmacy Desk</p>
            </div>
            <div className="w-[42px] h-[42px] rounded-full overflow-hidden shadow-sm border-2 border-white group-hover:border-[#6C5CE7] transition-colors bg-gradient-to-tr from-rose-200 to-orange-200 flex items-center justify-center text-rose-600 font-bold text-[14px]">
              PH
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 p-6 lg:p-10 max-w-7xl mx-auto w-full">
        ...
        >
          <div className="p-8 border-b border-[#EEF2FF]">
            <h2 className="text-[20px] font-black text-slate-800 mb-1">Pharmacy Queue</h2>
            <p className="text-[13px] text-slate-500 font-medium">Manage prescriptions sent by doctors</p>
            
            <div className="flex items-center gap-6 mt-8">
              {['All', 'Pending', 'Waiting', 'Completed'].map(tab => (
                <button 
                  key={tab} 
          ...
          <div className="flex-1 overflow-auto">
            {filteredOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-400 pb-10">
                ...
                <p className="text-[12px]">New prescriptions will appear here automatically.</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50">
                    ...
                    <th className="py-4">Medicines</th>
                          ...
                          <div className="space-y-1">
                            {order.medicines.map((m, i) => (
                              <p key={i} className="text-[12px] font-semibold text-slate-600 flex items-center gap-1.5">
                                ...
                                {m.name} <span className="text-[10px] text-slate-400 font-medium">({m.dosage})</span>
                              </p>
                            ))}
                          </div>
                        </td>
